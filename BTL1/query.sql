...
UNION
SELECT * FROM CN2.HOADON@cn2_dblink H2
WHERE NOT EXISTS (
SELECT * FROM CN2.SANPHAM@cn2_dblink S2
WHERE S2.NUOCSX = 'Viet Nam' AND NOT EXISTS (
SELECT * FROM CN2.CTHD@cn2_dblink C2
...
SELECT S.MASP, S.TENSP, S.NUOCSX, MAX(KHUYENMAI) AS "KHUYENMAI_CAONHAT"
FROM (SELECT *
...
GROUP BY S.MASP, S.TENSP, S.NUOCSX;

--Cau 8: Tinh tong cac hoa don o 2 chi nhanh 
SELECT C.MACHINHANH, SUM(TRIGIA) AS "TONG_THU_NHAP"
FROM (SELECT *
FROM CN1.CTHD T1 JOIN CN1.HOADON H1 ON T1.SOHD = H1.SOHD
UNION
SELECT *
FROM CN2.CTHD@cn2_dblink T2 JOIN CN2.HOADON@cn2_dblink H2 ON T2.SOHD = H2.SOHD ) D 
JOIN CN1.CHINHANH C 
ON D.MACHINHANH = C.MACHINHANH
GROUP BY C.MACHINHANH
ORDER BY C.MACHINHANH;

--Cau 9: Gia trung binh cua cac loai san pham tai 2 chi nhanh
SELECT S.TENSP, AVG(GIA) AS "GIA_TRUNG BINH"
FROM (SELECT * 
FROM CN1.SANPHAM 
UNION
SELECT * 
FROM CN2.SANPHAM@cn2_dblink) S
GROUP BY S.TENSP
ORDER BY S.TENSP;
