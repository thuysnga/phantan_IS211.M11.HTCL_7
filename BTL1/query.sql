...
FROM CN2.CTHD@cn2_dblink C2 JOIN CN2.SANPHAM@cn2_dblink S2 ON C2.MASP = S2.MASP
WHERE C2.SOLUONG > 5;
...
UNION
SELECT S2.MASP, S2.TENSP
FROM CN2.KHOSANPHAM_QLBH@cn2_dblink B2 JOIN CN2.SANPHAM@cn2_dblink S2 ON B2.MASP = S2.MASP
...
FROM CN2.HOADON@cn2_dblink H2 JOIN CN2.KHACHHANG@cn2_dblink K2 ON H2.MAKH = K2.MAKH;
...
=======
--Cau 5: Tim san pham chi het hang o chi nhanh 1
SELECT S1.MASP, S1.TENSP 
FROM CN1.KHOSANPHAM_QLBH B1 JOIN CN1.SANPHAM S1 ON B1.MASP = S1.MASP 
WHERE B1.TINHTRANG = 'HET HANG' AND NOT EXISTS  
                  (SELECT S2.MASP, S2.TENSP  
                  FROM CN2.KHOSANPHAM_QLBH@DB_LINK B2 JOIN CN2.SANPHAM@DB_LINK S2 ON B2.MASP = S2.MASP 
                  WHERE B2.TINHTRANG = 'HET HANG'); 
>>>>>>> e765a0229f43257a997b23df75b0c693c7fbc777
...
UNION
SELECT *
FROM CN2.NHANVIEN@cn2_dblink ) N JOIN CN1.CHINHANH C ON N.MACHINHANH = C.MACHINHANH
...
UNION
SELECT *
FROM CN2.KHOSANPHAM_QLBH@cn2_dblink ) B JOIN CN1.SANPHAM S ON B.MASP = S.MASP
...
UNION
SELECT *
FROM CN2.CTHD@cn2_dblink T2 JOIN CN1.HOADON H2 ON T2.SOHD = H2.SOHD ) D JOIN CN1.CHINHANH C ON D.MACHINHANH = C.MACHINHANH
GROUP BY C.MACHINHANH;
...
FROM CN2.HOADON@cn2_dblink H2 JOIN CN2.CHINHANH@cn2_dblink C2 ON H2.MACHINHANH = C2.MACHINHANH 
